...
class AsmaraTlahTerKalibrasi:
    ...
    def open_image1(self):
        ...
        self.hist1 = cv2.calcHist([image1], [0], None, [256], [0, 256])
        plt.figure(figsize=(8,6))
        plt.plot(self.hist1, color='k')
        plt.title("Histogram Gambar 1")
        plt.show()
        
        height, width, _ = image1.shape

        size = round((len(image1.tobytes()) / 1024), 2)

        top = tk.Toplevel()
        top.title("Property Gambar 1")
        top.geometry("300x150")

       
        label_path = ttk.Label(top, text=f"File Path: {self.filename1}")
        label_path.pack(pady=5)

        label_size = ttk.Label(top, text=f"Ukuran: {size} KB")
        label_size.pack()

        label_width = ttk.Label(top, text=f"Lebar: {width}")
        label_width.pack()

        label_height = ttk.Label(top, text=f"Tinggi: {height}")
        label_height.pack()
    ...
    def open_image2(self):
        ...
        self.hist2 = cv2.calcHist([image2], [0], None, [256], [0, 256])
        plt.figure(figsize=(8,6))
        plt.plot(self.hist2, color='k')
        plt.title("Histogram Gambar 2")
        plt.show()
        
        height, width, _ = image2.shape

        size = round((len(image2.tobytes()) / 1024), 2)

        top = tk.Toplevel()
        top.title("Property Gambar 2")
        top.geometry("300x150")

       
        label_path = ttk.Label(top, text=f"File Path: {self.filename2}")
        label_path.pack(pady=5)
    ...
    def image_subtraction(self):
        ...
        self.histSub = cv2.calcHist([subtracted_image], [0], None, [256], [0, 256])
        plt.figure(figsize=(8,6))
        plt.plot(self.histSub, color='k')
        plt.title("Histogram Substraksi")
        plt.show()
        ...
        cv2.imwrite(file_path, subtracted_image)
